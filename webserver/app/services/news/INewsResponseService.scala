...
trait INewsResponseService {
  def news(uid: Long): Future[Seq[NewsFeedResponse]]
}

class NewsResponseService @Inject() (val newsDAO: NewsResponseDao, val topicListDAO: TopicListDAO, val adResponseService: AdResponseService,
                                     val newsRecommendReadDAO: NewsRecommendReadDAO, val newsFeedDao: NewsFeedDao, val topicNewsReadDAO: TopicNewsReadDAO) extends INewsResponseService {

  def news(uid: Long): Future[Seq[NewsFeedResponse]] = {
    {
      val reuslt: Future[Seq[NewsFeedResponse]] = newsDAO.news(uid).map { seq =>
        seq.map { news =>
  ...
  def refreshFeedWithAd(uid: Long, page: Long, count: Long, timeCursor: Long, adbody: String, t: Int, remoteAddress: Option[String], v: Option[Int]): Future[Seq[NewsFeedResponse]] = {
    {
      ...
      //----热点部分----
      //有评论
      val refreshHotFO = newsDAO.hot((page - 1) * count, level2, newTimeCursor, uid)
      //百度热词
      val refreshHotWordFO = newsDAO.baiduHotWord((page - 1) * count, level2, newTimeCursor, uid)

      //----推荐部分----
      //模型推荐
      val refreshLDARecommendFO = newsDAO.byLDA((page - 1) * count, level1 / 2, newTimeCursor, uid)
      val refreshKMeansRecommendFO = newsDAO.byKmeans((page - 1) * count, level1, newTimeCursor, uid)
      //人工推荐
      val refreshByPeopleRecommendFO = newsDAO.byPeopleRecommend((page - 1) * count, level2 + 1, newTimeCursor, uid)

      //-------普通新闻------(可能情况:其他都没有数据,全出普通)
      val refreshCommonFO = newsDAO.common((page - 1) * count, count, newTimeCursor, uid)

      //----大图部分----
      //大图新闻
      val refreshBigImgFO5 = newsDAO.byBigImage((page - 1) * count, level4, newTimeCursor, uid)
      ...
      //视频
      val videoFO = newsDAO.video((page - 1) * count, level4, newTimeCursor, uid)
  ...
  def toNewsFeedResponse(nid: Long, url: String, docid: String, title: String, pname: Option[String], purl: Option[String],
    ...
    val newsSimpleRowBase = NewsSimpleRowBase(Some(nid), url, docid, title, None, LocalDateTime.fromDateFields(ctime), pname, purl, None, None)
    val newsSimpleRowIncr = NewsSimpleRowIncr(collect, concern, comment, inum, style, imgsList)
    val newsSimpleRowSyst = NewsSimpleRowSyst(state, LocalDateTime.fromDateFields(ctime), chid, None, icon, rtype, videourl, thumbnail, duration)
