...
class NewsResponseService @Inject() (val newsResponseDao: NewsResponseDao, val topicListDAO: TopicListDAO, val adResponseService: AdResponseService,
                                     val newsFeedDao: NewsFeedDao, val topicNewsReadDAO: TopicNewsReadDAO) extends INewsResponseService {

  def news(uid: Long): Future[Seq[NewsFeedResponse]] = {
    {
      val reuslt: Future[Seq[NewsFeedResponse]] = newsResponseDao.news(uid).map { seq =>
        seq.map { news =>
