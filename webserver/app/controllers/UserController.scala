...
class UserController @Inject() (val userService: UserService)(implicit ec: ExecutionContext)
   ...
   */
  def signupSocial() = Action.async(parse.json) { implicit request =>
    request.body.validate[UserSocial] match {
      ...
      case JsSuccess(userSocial, _) if userSocial.muid.isDefined =>
        userService.updateUserRowByUid(userSocial.muid.get, UserRowHelpers.from(userSocial)).flatMap {
          case Some(userRow) => gotoLoginSucceededBuilder(userRow)
   ...
   */
  def signupGuest() = Action.async(parse.json) { implicit request =>
   ...
   */
  def loginGuest() = Action.async(parse.json) { implicit request =>
  ...
  private def gotoLoginSucceededBuilder(userRow: UserRow)(implicit request: RequestHeader): Future[Result] = {
    val userRep: UserResponse = UserRowHelpers.toResponse(userRow)
    val uid: Long = userRep.uid
    gotoLoginSucceeded(uid, Future.successful(ServerSucced(userRep)))
  }
