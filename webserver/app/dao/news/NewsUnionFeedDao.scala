...
object NewsUnionFeedDao {
  ...
  final private val condition: String = " and nv.chid != 28 and nv.state=0 and nv.pname not in('就是逗你笑', 'bomb01') and nv.sechid is null and (nv.rtype is null  or nv.rtype=0) "
...
class NewsUnionFeedDao @Inject() (@NamedDatabase("pg2") protected val dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext)
  ...
  def byLDAandKmeans(offset: Long, limit: Long, timeCursor: LocalDateTime, uid: Long): Future[Seq[NewsFeedRow]] = {
    ...
    val limitalgorithm = limit / 3
    val action = sql"select * from ( #$select , 21 as rtype, 21 as logtype from newslist_v2 nv where  not exists (select 1 from #$tablename1 nr where nv.nid=nr.nid and nr.uid=$uid and nr.readtime>#$dayWindow1) and nv.nid in (select nid from #$tablename2 where uid=$uid and ctime>#$dayWindow1 and sourcetype=1) and nv.ctime>#$dayWindow1 #$condition limit $limitalgorithm )lda union all select * from (#$select , 21 as rtype, 22 as logtype from newslist_v2 nv where  not exists (select 1 from #$tablename1 nr where nv.nid=nr.nid and nr.uid=$uid and nr.readtime>#$dayWindow1) and nv.nid in (select nid from #$tablename2 where uid=$uid and ctime>#$dayWindow1 and sourcetype=2) and nv.ctime>#$dayWindow1 #$condition limit $limitalgorithm)kmeans union all select * from (#$select , 21 as rtype, 27 as logtype from newslist_v2 nv where  not exists (select 1 from #$tablename1 nr where nv.nid=nr.nid and nr.uid=$uid and nr.readtime>#$dayWindow1) and nv.nid in (select nid from #$tablename2 where uid=$uid and ctime>#$dayWindow1 and sourcetype=3) and nv.ctime>#$dayWindow1 #$condition limit $limitalgorithm)cf ".as[NewsFeedRow]
    db.run(action)
  }
  ...
  def byBigImageAndVideo(offset: Long, limit: Long, timeCursor: LocalDateTime, uid: Long): Future[Seq[NewsFeedRow]] = {
    val tablename: String = "newsrecommendread_" + uid % 100
    val action = sql" select * from (select nv.nid, nv.docid, nv.title, nv.pname, nv.purl, nv.chid, nv.collect, nv.concern, nv.un_concern, nv.comment, case when nr.bigimg>0 then (10+nr.bigimg) else nv.style end as style, array_to_string(nv.imgs, ',') as imgs,  nv.icon, nv.videourl, nv.duration, nv.thumbnail, nv.clicktimes, 999 as rtype, 25 as logtype from newslist_v2 nv inner join newsrecommendlist nr on nv.nid=nr.nid where  not exists (select 1 from #$tablename nr where nv.nid=nr.nid and nr.uid=$uid and nr.readtime>#$dayWindow3)  and nv.ctime>#$dayWindow3 and nr.rtime>#$dayWindow3 order by level desc, style desc, rtime desc limit $limit)bigimage union all select * from(#$select , 6 as rtype, 6 as logtype from newslist_v2 nv where  not exists (select 1 from #$tablename nr where nv.nid=nr.nid and nr.uid=$uid and nr.readtime>#$dayWindow1) and nv.ctime>#$dayWindow1 and nv.chid=44  and nv.rtype=6 limit $limit)video ".as[NewsFeedRow]
    db.run(action)
  }
  ...
  def byBigImage(offset: Long, limit: Long, timeCursor: LocalDateTime, uid: Long): Future[Seq[NewsFeedRow]] = {
    val tablename: String = "newsrecommendread_" + uid % 100
    val action = sql" select nv.nid, nv.docid, nv.title, nv.pname, nv.purl, nv.chid, nv.collect, nv.concern, nv.un_concern, nv.comment, case when nr.bigimg>0 then (10+nr.bigimg) else nv.style end as style, array_to_string(nv.imgs, ',') as imgs,  nv.icon, nv.videourl, nv.duration, nv.thumbnail, nv.clicktimes, 999 as rtype, 25 as logtype from newslist_v2 nv inner join newsrecommendlist nr on nv.nid=nr.nid where  not exists (select 1 from #$tablename nr where nv.nid=nr.nid and nr.uid=$uid and nr.readtime>#$dayWindow3)  and nv.ctime>#$dayWindow3 and nr.rtime>#$dayWindow3  order by level desc, style desc, rtime desc limit $limit ".as[NewsFeedRow]
    db.run(action)
  }
