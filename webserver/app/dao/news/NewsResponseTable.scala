...
object NewsResponseDao {
  ...
  final private val select: String = "select nv.nid, nv.docid, nv.title, nv.pname, nv.purl, nv.chid, nv.collect, nv.concern, nv.un_concern, nv.comment, nv.style, array_to_string(nv.imgs, ',') as imgs,  nv.icon, nv.videourl, nv.duration, nv.thumbnail, nv.rtype "
...
class NewsResponseDao @Inject() (@NamedDatabase("pg2") protected val dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext)
  ...
  def byChannel(offset: Long, limit: Long, timeCursor: LocalDateTime, uid: Long, chid: Long): Future[Seq[NewsFeedRow]] = {
    ...
    val action = sql" #$select , 0 as logtype from newslist_v2 nv where  not exists (select 1 from #$tablename nr where nv.nid=nr.nid and nr.uid=$uid and nr.readtime>#$dayWindow3) and nv.chid=$chid and nv.ctime>#$dayWindow3  #$condition offset $offset limit $limit ".as[NewsFeedRow]
    db.run(action)
  }

  def bySeChannel(offset: Long, limit: Long, timeCursor: LocalDateTime, uid: Long, chid: Long, sechid: Long): Future[Seq[NewsFeedRow]] = {
    val tablename: String = "newsrecommendread_" + uid % 100
    val action = sql" #$select , 0 as logtype from newslist_v2 nv where  not exists (select 1 from #$tablename nr where nv.nid=nr.nid and nr.uid=$uid and nr.readtime>#$dayWindow3) and nv.chid=$chid and nv.sechid=$sechid and nv.ctime>#$dayWindow3  #$condition offset $offset limit $limit ".as[NewsFeedRow]
    db.run(action)
  }

  def byChannelWithKmeans(offset: Long, limit: Long, timeCursor: LocalDateTime, uid: Long, chid: Long): Future[Seq[NewsFeedRow]] = {
    val tablename: String = "newsrecommendread_" + uid % 100
    val action = sql" #$select , 22 as logtype from newslist_v2 nv where nv.chid=$chid and nv.ctime>#$dayWindow3 and nv.nid in (select nid from news_kmeans nk inner join user_kmeans_cluster ukc on nk.model_v=ukc.model_v and nk.cluster_id=ukc.cluster_id and nk.chid=ukc.chid where not exists (select 1 from #$tablename nr where nk.nid=nr.nid and nr.uid=$uid and nr.readtime>#$dayWindow3) and ukc.uid=$uid and nk.ctime>#$dayWindow3 and ukc.chid=$chid order by random() limit $limit) ".as[NewsFeedRow]
    db.run(action)
  }

  def video(offset: Long, limit: Long, timeCursor: LocalDateTime, uid: Long): Future[Seq[NewsFeedRow]] = {
    val tablename: String = "newsrecommendread_" + uid % 100
    val action = sql" #$select , 6 as logtype from newslist_v2 nv where  not exists (select 1 from #$tablename nr where nv.nid=nr.nid and nr.uid=$uid and nr.readtime>#$hourWindow24) and nv.ctime>#$hourWindow24  and nv.rtype=6 limit $limit ".as[NewsFeedRow]
    db.run(action)
  }
