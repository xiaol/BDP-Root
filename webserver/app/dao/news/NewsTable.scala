...
trait NewsTable { self: HasDatabaseConfig[MyPostgresDriver] =>
  ...
  class NewsTable(tag: Tag) extends Table[NewsRow](tag, "newslist_v2") {
    ...
    def base = (nid.?, url, docid, title, author, ptime, pname, purl, province, city, district) <> ((NewsRowBase.apply _).tupled, NewsRowBase.unapply)
    def incr = (collect, concern, un_concern, comment, inum, style, imgs) <> ((NewsRowIncr.apply _).tupled, NewsRowIncr.unapply)
    def syst = (state, ctime, chid, sechid, srid, icon, rtype, videourl, thumbnail, duration) <> ((NewsRowSyst.apply _).tupled, NewsRowSyst.unapply)
